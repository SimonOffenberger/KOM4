...
int Adc_Initialize()
{
  ...
  // Enable peripheral clock and reset peripheral registers.
  ...
  VREFINT_DATA = (uint_fast16_t)value;
...
int Adc_Convert(const int id, float * const value,const bool raw)
{
	
	// value can be nullptr
	if(value==0) return ERROR_NULLPTR;
  ...
  switch(id)
  {
    case ADC_ID_TEMPERATURE:
      *value = 90.0F / (TS_CAL2 - TS_CAL1) * (dr - TS_CAL1) + 30.0F;
      break;
      
    default:
      *value = VREFINT_CAL * 3.3F * dr / ((float)VREFINT_DATA * 1.0F * 0x0FFF);
      break;
  }
